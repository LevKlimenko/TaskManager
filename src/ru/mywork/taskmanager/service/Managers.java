...
public class Managers {
    ...
    public static void main(String[] args) throws ManagerSaveException, IOException {
        ...
        Subtask subtask3 = new Subtask("Посуда", "Помыть посуду", epic1.getId(), Status.IN_PROGRESS);
        manager.addNewSubTask(subtask3);
        manager.printById(epic1.getId());
        manager.printById(subtask1.getId());
        manager.printById(task1.getId());
        manager.printHistory();
        System.out.println(manager.getHistory1());

        FileBackedTaskManager managers = FileBackedTaskManager.loadFromFile(new File("tasks.csv"));
        System.out.println("----История----");
        System.out.println(managers.getHistory1());
        System.out.println("----Конец----");
        managers.printById(epic1.getId());
        managers.printById(task1.getId());
        managers.printById(epic1.getId());
        managers.printById(subtask1.getId());
